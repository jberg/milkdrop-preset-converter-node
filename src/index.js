...
function optimizePresetEquations(preset) {
  ...
  presetMap.init_eqs_str = presetMap.init_eqs_str
    ? optimizeEquations(presetMap.init_eqs_str)
    : "";
  presetMap.frame_eqs_str = presetMap.frame_eqs_str
    ? optimizeEquations(presetMap.frame_eqs_str)
    : "";
  presetMap.pixel_eqs_str = presetMap.pixel_eqs_str
    ? optimizeEquations(presetMap.pixel_eqs_str)
    : "";

  for (let i = 0; i < presetMap.shapes.length; i++) {
    if (presetMap.shapes[i].baseVals.enabled !== 0) {
      presetMap.shapes[i].init_eqs_str = presetMap.shapes[i].init_eqs_str
        ? optimizeEquations(presetMap.shapes[i].init_eqs_str)
        : "";
      presetMap.shapes[i].frame_eqs_str = presetMap.shapes[i].frame_eqs_str
        ? optimizeEquations(presetMap.shapes[i].frame_eqs_str)
        : "";
    }
  }
  ...
  for (let i = 0; i < presetMap.waves.length; i++) {
    if (presetMap.waves[i].baseVals.enabled !== 0) {
      presetMap.waves[i].init_eqs_str = presetMap.waves[i].init_eqs_str
        ? optimizeEquations(presetMap.waves[i].init_eqs_str)
        : "";
      presetMap.waves[i].frame_eqs_str = presetMap.waves[i].frame_eqs_str
        ? optimizeEquations(presetMap.waves[i].frame_eqs_str)
        : "";
      presetMap.waves[i].point_eqs_str = presetMap.waves[i].point_eqs_str
        ? optimizeEquations(presetMap.waves[i].point_eqs_str)
        : "";
    }
  }
...
export function convertShaders(presetParts) {
  ...
  presetMap.init_eqs_eel_str = presetParts.presetInit
    ? presetParts.presetInit.trim()
    : "";
  presetMap.frame_eqs_eel_str = presetParts.perFrame
    ? presetParts.perFrame.trim()
    : "";
  presetMap.pixel_eqs_eel_str = presetParts.perVertex
  ...
  for (let i = 0; i < presetParts.shapes.length; i++) {
    presetMap.shapes.push(
      _.assign(
        { baseVals: presetParts.shapes[i].baseVals },
        {
          init_eqs_eel_str: presetParts.shapes[i].init_eqs_str
            ? presetParts.shapes[i].init_eqs_str
            : "",
          frame_eqs_eel_str: presetParts.shapes[i].frame_eqs_str
            ? presetParts.shapes[i].frame_eqs_str
            : "",
        }
      )
    );
  ...
  for (let i = 0; i < presetParts.waves.length; i++) {
    presetMap.waves.push(
      _.assign(
        { baseVals: presetParts.waves[i].baseVals },
        {
          init_eqs_eel_str: presetParts.waves[i].init_eqs_str
            ? presetParts.waves[i].init_eqs_str
            : "",
          frame_eqs_eel_str: presetParts.waves[i].frame_eqs_str
            ? presetParts.waves[i].frame_eqs_str
            : "",
          point_eqs_eel_str: presetParts.waves[i].point_eqs_str
            ? presetParts.waves[i].point_eqs_str
            : "",
        }
      )
    );
  ...
  presetMap.warp = processOptimizedShader(
    convertPresetShader(presetParts.warp)
  );
  presetMap.comp = processOptimizedShader(
    convertPresetShader(presetParts.comp)
  );
...
export function convertPresetMap(presetParts, optimize = true) {
  ...
  if (
    (_.isEmpty(presetOutput.warp) && !_.isEmpty(presetParts.warp)) ||
